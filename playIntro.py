...
def playIntro(exp, video, audio, keyboard, allowSkip, language):
    """
    ...
    introMovie = config.introMovie
    introAudio = config.introAudio
    ...
    if not allowSkip:
        ...
        playWholeMovie(video, audio, introMovie, introAudio,clock, stopBc)
        video.unshow(shown)
        seenOnce = True
    else:
...
def playWholeMovie(video, audio, movieFile, audioFile, clock, bc = None):
    """
    Plays any movie file and audio file synchronously
    """
    movieObject = Movie(movieFile)
    movieObject.load()
    #sound = FileAudioClip(audioFile)
   # sound.load()
...
if __name__ == "__main__":
    ...
    playIntro(exp, video, audio, allowSkip, language)
